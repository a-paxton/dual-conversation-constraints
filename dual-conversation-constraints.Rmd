...
rio_residuals = qplot(x = fitted(rec_convers_condition_gca_st_rio), 
  ...
  scale_color_viridis() +
  coord_cartesian(xlim = c(-4,4), ylim = c(-2,2.5)) +
...
```{r create-surrogate-interactions, eval = FALSE}
...
```{r export-surrogate-dfs, eval = FALSE}
...
dual_task_plot = ggplot(real_dual_task_data,
  ...
  ylim(c(min(c(real_noise_data$rr,real_dual_task_data$rr)) - .01),
       max(c(real_noise_data$rr,real_dual_task_data$rr)) + .01) +
...
tiff('./figures/DCC-interaction-RR_lag_conversation_condition.tiff',
       units='in', res = 300, width = 4, height = 5)
grid.arrange(
         top=textGrob("Head movement synchrony\nby task and conversation",
                      gp=gpar(fontsize=14)),
         dual_task_plot,
...
# save high-resolution version
tiff('./figures/DCC-all_dyad_profiles.tiff',
